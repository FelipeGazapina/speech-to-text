...
```
 tap ⌥ ──► record mic ──► tap ⌥ ──► Whisper (local) ──► cleanup LLM (local, optional) ──► paste at cursor
```
...
[transcription]
model = "large-v3-turbo"  # "small.en" is faster on older/Intel Macs
language = "en"           # "pt" for Portuguese, "" for auto-detect

[cleanup]
model = "qwen2.5:3b"      # any Ollama model; try "gemma3:4b" or "llama3.2:3b"
vocabulary = ["Supabase", "Zod", "tRPC", "MyCompanyName"]   # names it should spell right
...
- **It transcribes but nothing gets pasted.** Accessibility isn't granted. The text is still available from **Copy last transcription**.
...
| `transcriber.py` | Whisper via MLX (Apple Silicon) or faster-whisper |
